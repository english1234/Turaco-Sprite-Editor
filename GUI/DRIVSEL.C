	...
	void (*callback)(char*, int, int), int param)
{
	...
	do {
		...
		if (lastSlash) {
			...
			safe_strncpy(fullpath, pattern, MAX_PATH);
			
			fullpath[dirLen] = '\0';
			strcat_s(fullpath, MAX_PATH, findData.cFileName);
		}
		else {
			safe_strncpy(fullpath, findData.cFileName, MAX_PATH);
		}
...
void fds_callback(char* fname, int attrib, int param)
{
	...
	if (!fds_total)
	{
		show_mouse(NULL, "fds_callback 1");
		progress_box();
		textout_centre(screen, font, "Counting...", 160, 100, GUI_FORE);
		show_mouse(screen, "fds_callback 2");
	}
	...
	if (param)
	{
		if (keypressed())
		{
			...
			safe_strncpy(fds[fds_count].gamename_long, "Skipped.", FDS_GN_LONG);
			safe_strncpy(fds[fds_count].gamename_short, "???", FDS_GN_SHORT);
			...
			strncpy_s(fds[fds_count].filename, FDS_FILENAME, fname, _TRUNCATE);
			...
			// Get description - use safe comparison
			char* description = get_config_string("General", "Description", "None");
			printf("DEBUG fds_callback: description = '%s'\n", description);

			// SAFE STRING COMPARISON: Check if description is valid and not "None"
			int has_valid_description = 0;
			if (description != NULL && description != (char*)(intptr_t)NULL) {
				printf("DEBUG: Comparing '%s' with 'None'\n", description);
				if (strcmp(description, "None") != 0) {
					has_valid_description = 1;
					printf("DEBUG: Valid description found: %s\n", description);
				}
			}

			if (has_valid_description)
			{
				printf("DEBUG: Copying description to gamename_long\n");
				safe_strncpy(fds[fds_count].gamename_long, description, FDS_GN_LONG);
				printf("DEBUG: gamename_long = '%s'\n", fds[fds_count].gamename_long);

				pBuffer = get_filename(fname);
				printf("DEBUG: filename = '%s'\n", pBuffer);
				safe_strncpy(fds[fds_count].gamename_short, pBuffer, FDS_GN_SHORT);
				printf("DEBUG: gamename_short = '%s'\n", fds[fds_count].gamename_short);

				// DECODE COUNT LOOP - with safe string handling
				count = 0;
				char* start_value;
				do {
					snprintf(sect, sizeof(sect), "Decode%d", count + 1);
					start_value = get_config_string(sect, "start", "----");

					// Safe comparison
					if (start_value != NULL && strcmp(start_value, "----") != 0) {
						count++;
					}

					// Safe free - only if it was allocated
					if (start_value != NULL && start_value != (char*)(intptr_t)NULL) {
						SAFE_FREE(start_value);
					}
				} while (start_value != NULL && strcmp(start_value, "----") != 0);
				fds[fds_count].n_banks = count - 1;

				// PALETTE COUNT LOOP - with safe string handling
				count = 0;
				char* palette_value;
				do {
					snprintf(sect, sizeof(sect), "Palette%d", count + 1);
					palette_value = get_config_string("Palette", sect, "----");

					// Safe comparison
					if (palette_value != NULL && strcmp(palette_value, "----") != 0) {
						count++;
					}

					// Safe free
					if (palette_value != NULL && palette_value != (char*)(intptr_t)NULL) {
						SAFE_FREE(palette_value);
					}
				} while (palette_value != NULL && strcmp(palette_value, "----") != 0);
				...
				// Free the description string safely
				if (description != NULL && description != (char*)(intptr_t)NULL) {
					SAFE_FREE(description);
				}
			}
			else {
				printf("DEBUG: No valid description, marking as badly formatted\n");
				safe_strncpy(fds[fds_count].gamename_long,  "Badly formatted file.", FDS_GN_LONG);
				safe_strncpy(fds[fds_count].gamename_short, "???", FDS_GN_SHORT);
				fds[fds_count].n_banks = 0;
				fds[fds_count].n_palettes = 0;
				fds[fds_count].planes = 0l;
		...
		{
			show_mouse(NULL, "fds_callback 3");
			// ... progress display code ...
			show_mouse(screen, "fds_callback 4");
		}
	}
}
...
void fds_dir_callback(char* fname, int attrib, int param)
{
	if (attrib == FA_DIREC)
			...
			strcmp(fname, "drivers\\.."))  // skip these
		{
			...
			dirlist[ndirs - 1] = (char*)debug_malloc(16, __FILE__, __LINE__);
			if (dirlist[ndirs - 1]) {
				safe_strncpy(dirlist[ndirs - 1], fname + 8, 16);
			}
			else {
				// Handle allocation failure
				printf("ERROR: Failed to allocate memory for directory list\n");
			}
		}
}
...
int d_my_list_proc(int msg, DIALOG* d, int c)
{
	...
	if (msg == MSG_START)
	{
		...
		if (file_exists("drivers", FA_DIREC, &aret))
		{
			...
			{
				SDL_snprintf(tdir, sizeof(tdir), "drivers\\%s\\*.ini", dirlist[cdir]);
		...
		{
			fds = (FDS*)SAFE_MALLOC(sizeof(FDS) * fds_total);
			if (fds)
			{
				...
				// now sort the list...
				qsort(fds, fds_total, sizeof(FDS), fds_cmp);
			}
		}
		else {
			not_busy();
	...
	if (msg == MSG_CHAR)
	{
		if (isalnum(c))
		{
			...
			// SAFETY CHECK: Ensure fds is valid and d->d1 is in bounds
			if (fds && d->d1 >= 0 && d->d1 < fds_total)
			{
	...
	if (msg == MSG_DCLICK)
	{
		// SAFETY CHECK: Ensure selected_item is valid before accessing fds array
		if (fds && selected_item >= 0 && selected_item < fds_total)
		{
			busy();
			SDL_strlcpy(selected_filename, fds[selected_item].filename, sizeof(selected_filename));
			return D_EXIT;
		}
		return D_O_K;
	}
	...
	if (msg != MSG_IDLE) {
		...
		// SAFETY CHECK: Ensure fds is valid and d->d1 is in bounds before accessing
		if (fds && d->d1 >= 0 && d->d1 < fds_total)
		{
		...
		{
			// Display safe default values when index is out of bounds
			textprintf(screen, font, text_x, text_y, GUI_FORE, "%-12s", "--------");
			textprintf(screen, font, text_x, text_y + 16, GUI_FORE, "%-12s", "--------");
			textprintf(screen, font, text_x, text_y + 36, GUI_FORE, "--------");
		}
...
{
	busy();
	safe_strncpy(selected_filename, fds[selected_item].filename, sizeof(selected_filename));
	return D_EXIT;
}
